...
static int on_keymap_binding_pressed(struct zmk_behavior_binding *binding,
    ...
    return raise_zmk_mouse_move_state_changed_from_encoded(binding->param1, *config, true, event.timestamp);
}
...
static int on_keymap_binding_released(struct zmk_behavior_binding *binding,
    ...
    const struct device *dev = device_get_binding(binding->behavior_dev);
    const struct mouse_config *config = dev->config;
    return raise_zmk_mouse_move_state_changed_from_encoded(binding->param1, *config,
